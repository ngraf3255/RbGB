...
impl CTC {
    ...
    pub fn write(&mut self, bus: &dyn Bus, chn: usize, val: Word) {
        ...
        if (old_ctrl & CTC_CONSTANT_FOLLOWS) == CTC_CONSTANT_FOLLOWS {
            ...
            for i in 0..NUM_CHANNELS {
                self.chn[i].int_vector = ((val & 0xF8) + 2u16 * i as u16) as u8;
            }
        }
    ...
    pub fn read(&self, chn: usize) -> Word {
        let c = self.chn[chn];
        let mut val = c.down_counter as Word;
        ...
        for chn in 0..NUM_CHANNELS {
            ...
            if (ctrl & (CTC_RESET | CTC_CONSTANT_FOLLOWS)) == 0 {
                if (ctrl & CTC_MODE_BIT) == CTC_MODE_TIMER && !waiting {
                    self.chn[chn].down_counter -= cycles as Word;
    ...
    fn down_counter_initial(c: &Channel) -> Word {
        let mut val: Word = if 0 == c.constant {
            0x100
        } else {
            c.constant as Word
        };
    ...
    fn down_counter_trigger(&self, bus: &dyn Bus, chn: usize) {
        if (self.chn[chn].control & CTC_INTERRUPT_BIT) == CTC_INTERRUPT_ENABLED {
            bus.ctc_irq(self.id, chn, self.chn[chn].int_vector as Word);
        }
...
mod test {
    ...
    impl Bus for TestBus {
        ...
        fn ctc_irq(&self, ctc: usize, chn: usize, int_vector: Word) {
            let mut state = self.state.borrow_mut();

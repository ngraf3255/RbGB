...
pub trait Bus {
    ...
    fn cpu_inp(&self, port: Word) -> Byte {
        0
    }
    /// CPU writes to I/O port
    fn cpu_outp(&self, port: Word, val: Byte) {}
    ...
    /// PIO output callback
    fn pio_outp(&self, pio: usize, chn: usize, data: Word) {}
    /// PIO input callback
    fn pio_inp(&self, pio: usize, chn: usize) -> Word {
        0
    }
    /// PIO channel rdy line has changed
    fn pio_rdy(&self, pio: usize, chn: usize, rdy: bool) {}
    /// interrupt request from PIO
    fn pio_irq(&self, pio: usize, chn: usize, int_vector: Word) {}
